...
class DummyBot:
    ...
    async def reply_to(self, message, text, **kwargs):
        ...
        return SimpleNamespace(
            chat=SimpleNamespace(id=message.chat.id), message_id=len(self.replies)
        )
...
def _make_message(text: str):
    return SimpleNamespace(
        ...
        from_user=SimpleNamespace(
            id=42, first_name="Alice", last_name="", username="alice"
        ),
    )
...
def test_extract_message_question_strips_bot_mention(monkeypatch):
    ...
    question = memory_module._extract_message_question(
        "@NachoNekoX_bot 帮我找一下服务器维护时间"
    )
...
def test_dispatch_list_is_allowed_for_non_admin(monkeypatch):
    ...
    monkeypatch.setattr(
        memory_module, "_load_entries", lambda chat_id: _sample_entries()
    )
...
def test_execute_update_memory_updates_entry(monkeypatch):
    ...
    monkeypatch.setattr(
        memory_module,
        "_save_entries",
        lambda chat_id, chat_title, updated_entries: saved.setdefault(
    ...
    asyncio.run(
        memory_module._execute_update_memory(
            bot, msg, "MEM-0002", "维护时间改到周六 03:00", "MEM-0002"
        )
    )
...
def test_parse_intent_falls_back_when_ai_fails(monkeypatch):
    ...
    parsed = asyncio.run(
        memory_module._parse_intent("删掉关于服务器维护的备忘", True, True)
    )
