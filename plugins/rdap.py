...
async def handle_rdap_command(bot, message: types.Message):
    ...
    msg = await bot.reply_to(message, f"正在查询 {data} RDAP 信息...", disable_web_page_preview=True)
...
async def register_handlers(bot, middleware, plugin_name):
    ...
    middleware.register_inline_handler(
        ...
        priority=50,
        stop_propagation=True,
        func=lambda q: bool(getattr(q, 'query', None)) and q.query.strip().lower().startswith('rdap')
    )
