...
def _get_memory_config() -> dict[str, Any]:
    ...
    return {
        ...
        "intent_confidence_threshold": float(
            conf.get("intent_confidence_threshold", 0.68)
        ),
    }
...
def _score_match(query: str, entry: MemoryEntry) -> float:
    ...
    haystack = " ".join(
        [entry.memory_id, entry.title, entry.body, " ".join(entry.keywords)]
    )
    normalized_haystack = _normalize_text(haystack)
    ratio = SequenceMatcher(None, normalized_query, normalized_haystack).ratio()
    if not normalized_query:
        return 0.0
    overlap = len(_tokenize(normalized_query) & _tokenize(normalized_haystack)) / max(
        len(_tokenize(normalized_query)), 1
    )
...
def _parse_memory_entries(content: str) -> list[MemoryEntry]:
    ...
    for match in ENTRY_RE.finditer(content or ""):
        keywords = [
            item.strip() for item in (match.group(7) or "").split(",") if item.strip()
        ]
        entries.append(
            MemoryEntry(
...
def _extract_message_question(text: str) -> str:
    ...
    if not username or not text:
        return ""
    return re.sub(
        rf"@{re.escape(username)}\b", "", text, count=1, flags=re.IGNORECASE
    ).strip(" \n\t:,-")
...
async def _reply_or_edit(
    bot, message: types.Message, text: str, progress_message=None, **kwargs
):
...
async def _call_openai_compatible(messages: list[dict[str, Any]]) -> str:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        async with session.post(
            f"{conf['base_url']}/chat/completions", headers=headers, json=payload
        ) as resp:
...
def _fallback_optimize_memory(text: str) -> dict[str, Any]:
    ...
    return {
        "title": cleaned[:30] or "Untitled",
        "body": cleaned or "",
        "keywords": [item for item in _tokenize(cleaned) if len(item) >= 2][:6],
    }
...
def _fallback_parse_intent(text: str) -> IntentParseResult:
    ...
    if any(hint in text for hint in UPDATE_HINTS):
        ...
        return IntentParseResult(
            "update_memory", target, memory_id, updated, 0.86, "rule:update"
        )
    if any(hint in text for hint in DELETE_HINTS):
        return IntentParseResult(
            "delete_memory",
            _trim_after_prefix(text, DELETE_HINTS),
            memory_id,
            "",
            0.86,
            "rule:delete",
    ...
    if any(hint in text for hint in ADD_HINTS):
        return IntentParseResult(
            "add_memory",
            _trim_after_prefix(text, ADD_HINTS),
            None,
            "",
            0.86,
            "rule:add",
...
async def _parse_intent_with_ai(
    raw_text: str, is_admin: bool, has_entries: bool
) -> IntentParseResult:
    messages = [
        {
            "role": "system",
...
async def _optimize_memory_text(
    ...
    recent_context = "\n".join(
        f"- {entry.memory_id}: {entry.title} | {entry.summary}"
...
def _find_duplicate_candidates(
    optimized: dict[str, Any], entries: list[MemoryEntry], threshold: float
...
def _find_candidates(
    query: str, entries: list[MemoryEntry], threshold: float, limit: int = 5
) -> list[tuple[MemoryEntry, float]]:
...
def _select_context_entries(
    query: str, entries: list[MemoryEntry], limit: int
) -> list[MemoryEntry]:
    scored = [(entry, _score_match(query, entry)) for entry in entries]
    scored.sort(key=lambda item: item[1], reverse=True)
...
async def _answer_from_memory(question: str, entries: list[MemoryEntry]) -> str:
    context_entries = _select_context_entries(
        question, entries, _get_memory_config()["max_context_items"]
    )
    if not context_entries:
        return _msg(
            "qa.no_match",
            "No matching group memory was found, so I can't answer only from group memories.",
        )
    context_text = "\n\n".join(
    ...
    messages = [
        {
            "role": "system",
            "content": "Answer only from the provided group memories. If the answer is missing, say so clearly.",
        },
        {
            "role": "user",
            "content": f"Memories:\n{context_text}\n\nQuestion:\n{question}",
    ...
    try:
        ...
        return _msg(
            "qa.ai_failed",
            "I found related memories, but AI failed:\n{bullets}",
            bullets=bullets,
        )
...
def _save_entries(chat_id: int, chat_title: str, entries: list[MemoryEntry]) -> None:
    _get_memory_file(chat_id).write_text(
        _render_memory_file(chat_title, entries), encoding="utf-8"
    )
...
async def _is_group_admin(bot, message: types.Message) -> bool:
    ...
    return await has_group_admin_permission(
        bot, message.chat.id, sender.id, required_permission=None
    )
...
async def _require_group_admin(bot, message: types.Message) -> bool:
    if message.chat.type not in ("group", "supergroup"):
        await bot.reply_to(
            message,
            _msg("error.group_only", "This feature can only be used in groups."),
        )
        return False
    if not await _is_group_admin(bot, message):
        await bot.reply_to(
            ...
            _msg(
                "error.modify_denied",
                "You can ask questions or view memories, but you cannot modify group memories.",
            ),
        )
        return False
    return True
...
async def _execute_add_memory(
    bot, message: types.Message, payload: str, progress_message=None
):
    ...
    if not payload.strip():
        await _reply_or_edit(
            ...
            message,
            _msg(
                "prompt.add_missing", "Please tell me what to save after mentioning me."
            ),
    ...
    progress = progress_message or await bot.reply_to(
        message, _msg("status.adding", "Saving group memory...")
    )
    ...
    duplicates = _find_duplicate_candidates(
        optimized, entries, _get_memory_config()["dedupe_threshold"]
    )
    entries.append(new_entry)
    _save_entries(message.chat.id, getattr(message.chat, "title", ""), entries)
    lines = [
        _msg(
            "result.added", "Added memory `{memory_id}`", memory_id=new_entry.memory_id
        ),
    ...
    if duplicates:
        dup_text = "；".join(
            f"{item.memory_id}({score:.2f})" for item, score in duplicates
        )
        lines.append(
            _msg(
                "result.duplicate_hint",
                "Possible duplicate memories: {items}",
                items=dup_text,
            )
        )
    text = "\n".join(lines)
    try:
        await bot.edit_message_text(
            text, progress.chat.id, progress.message_id, parse_mode="Markdown"
        )
    except Exception:
...
async def _execute_list_memory(bot, message: types.Message, progress_message=None):
    ...
    if not entries:
        await _reply_or_edit(
            bot,
            message,
            _msg("result.list_empty", "There are no group memories yet."),
            progress_message=progress_message,
        )
        return
    lines = [
        _msg(
            "result.list_header",
            "There are {count} memories in this group:",
            count=len(entries),
        ),
        "",
    ]
...
async def _execute_delete_memory(
    ...
    if not query:
        await _reply_or_edit(
            ...
            message,
            _msg(
                "prompt.delete_missing",
                "Please tell me which memory to delete, or give a MEM-id directly.",
            ),
            progress_message=progress_message,
        )
        return
    ...
    if not entries:
        await _reply_or_edit(
            bot,
            message,
            _msg("result.delete_empty", "There are no memories to delete."),
            progress_message=progress_message,
        )
        return
    candidates = _find_candidates(
        query, entries, _get_memory_config()["delete_match_threshold"]
    )
    if not candidates:
        await _reply_or_edit(
            bot,
            message,
            _msg("result.no_match", "No matching memory was found."),
            progress_message=progress_message,
        )
        return
    if len(candidates) == 1 and (
        candidates[0][1] >= _get_memory_config()["delete_match_threshold"]
        or bool(memory_id and MEMORY_ID_RE.match(memory_id))
    ):
        target = candidates[0][0]
        _save_entries(
            message.chat.id,
            getattr(message.chat, "title", ""),
            [entry for entry in entries if entry.memory_id != target.memory_id],
        )
        await _reply_or_edit(
            bot,
            message,
            _msg(
                "result.deleted",
                "Deleted memory `{memory_id}` {title}",
    ...
    lines = [
        _msg(
            ...
            "Matched multiple memories. Please use a more specific keyword or a MEM-id:",
        ),
        "",
    ]
...
async def _execute_update_memory(
    ...
    if not query:
        await _reply_or_edit(
            ...
            message,
            _msg(
                "prompt.update_missing_target",
                "Please tell me which memory to modify, or give a MEM-id directly.",
            ),
            progress_message=progress_message,
        )
        return
    if not updated_text.strip():
        await _reply_or_edit(
            ...
            message,
            _msg(
                "prompt.update_missing_content",
                "Please tell me what the memory should be updated to.",
            ),
            progress_message=progress_message,
        )
        return
    ...
    if not entries:
        await _reply_or_edit(
            bot,
            message,
            _msg("result.update_empty", "There are no memories to update."),
            progress_message=progress_message,
        )
        return
    candidates = _find_candidates(
    ...
    if len(candidates) != 1 and not (memory_id and MEMORY_ID_RE.match(memory_id)):
        lines = [
            _msg(
                "result.multiple_matches_update",
                "Matched multiple memories. Please use a more specific keyword or a MEM-id for update:",
            ),
            "",
        ]
        for entry, score in candidates:
    ...
    await _reply_or_edit(
        ...
        _msg(
            "result.updated",
            "Updated memory `{memory_id}` {title}",
            memory_id=target.memory_id,
            title=optimized["title"],
        ),
        progress_message=progress_message,
...
async def _answer_qa(bot, message: types.Message, question: str, progress_message=None):
    ...
    if not entries:
        await _reply_or_edit(
            ...
            _msg(
                "qa.empty",
                "There are no group memories yet, so I can't answer from memory.",
            ),
            progress_message=progress_message,
        )
        return
    if progress_message is not None:
        try:
            await bot.edit_message_text(
                _msg(
                    "status.answering",
                    "Searching group memories and preparing an answer...",
                ),
                progress_message.chat.id,
                progress_message.message_id,
            )
...
async def _dispatch_intent(
    ...
    if parsed.intent == "add_memory":
        if not is_admin:
            ...
            return
        await _execute_add_memory(
            bot, message, parsed.target_text, progress_message=progress_message
        )
        return
...
async def _handle_memory_message(bot, message: types.Message):
    if message.chat.type not in ("group", "supergroup") or not _message_mentions_bot(
        message
    ):
        return
    ...
    if not text:
        await bot.reply_to(
            message,
            _msg(
                "prompt.empty_after_mention",
                "Please put a concrete request after mentioning me.",
            ),
        )
        return
    ...
    await _dispatch_intent(
        bot, message, parsed, is_admin, progress_message=progress_message
    )
