...
async def query_java_server(address: str) -> str:
    ...
    try:
        ...
        msg_out = []
        msg_out.append(f"🎮 Minecraft Java 版服务器状态")
        msg_out.append(f"━━━━━━━━━━━━━━━━━━━━")
...
async def query_bedrock_server(address: str) -> str:
    ...
    try:
        ...
        msg_out = []
        msg_out.append(f"🎮 Minecraft 基岩版服务器状态")
        msg_out.append(f"━━━━━━━━━━━━━━━━━━━━")
        msg_out.append(f"📍 服务器地址：{address}")
...
async def handle_mcstatus_command(bot, message: types.Message, server_type: str = "java"):
    """
    处理 MC 服务器状态查询命令
    :param bot: Bot 对象
    :param message: 消息对象
    ...
    if len(command_args) < 2:
        await bot.reply_to(message, "请提供服务器地址，格式：/mc 服务器地址:端口\n例如：/mc mc.hypixel.net")
        return
...
async def register_handlers(bot, middleware, plugin_name):
    ...
    middleware.register_inline_handler(
        ...
        priority=50,
        stop_propagation=True,
        func=lambda q: bool(getattr(q, 'query', None)) and q.query.strip().lower().split()[0] in ['mc', 'mcje', 'mcbe']
    )
